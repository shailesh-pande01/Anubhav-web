...
import { Home, Plus, User, LogOut, Settings, ShieldAlert, LogIn } from 'lucide-react';
    ...
    <aside className="sticky top-6 flex h-[calc(100vh-3rem)] w-[220px] lg:w-[240px] flex-col justify-between py-2 select-none">
      ...
      <div className="flex flex-col">
        ...
        <nav className="mt-8 flex flex-col gap-1.5" aria-label="Desktop primary navigation">
          ...
          {/* Authenticated Links: Create & Profile */}
          {!isGuest && (
            <>
              {/* Create Post Button */}
              <button
      ...
      {/* Bottom section: Settings, Theme & Logout / Guest Actions */}
          ...
          <>
            ...
            {onSignIn && (
              <button
                type="button"
                onClick={onSignIn}
                className="flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-[14px] font-medium text-[var(--calm-primary)] hover:bg-[var(--calm-surface)] transition-colors"
              >
                <LogIn size={18} />
                <span>Sign in</span>
              </button>
